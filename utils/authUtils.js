...
const Authorize = async (req, res, next) => {
    ...
    if (!bearerHeader || !bearerHeader.startsWith("Bearer ")) return res.status(401).json({ message: "Token not accepted" });
    ...
    try {
        ...
        if (err.message === "jwt expired") {
            ...
            const user = await userModel.findOne({ where: { id: jwt.decode(token).data.id, is_active: 1 } });
            ...
            try {
                ...
                return res.status(400).json({ message: "Refresh token invalid. Please login again." });
            }
        }
...
const getUserFromToken = async (req, res) => {
    const bearerHeader = req.headers["authorization"];
    if (!bearerHeader || !bearerHeader.startsWith("Bearer ")) return res.sendStatus(401); // Unauthorized if token is missing
